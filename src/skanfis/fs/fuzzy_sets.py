...
class Triangular_MF(MF_object):
    ...
    def __init__(self, a=0, b=0.5, c=1):
        ...
        if a > b:
            raise Exception(
                "Error in triangular fuzzy set: a=%.2f should be <= b=%.2f" % (a, b)
            )
        elif b > c:
            raise Exception(
                "Error in triangular fuzzy set: b=%.2f should be <= c=%.2f" % (b, c)
            )
...
class Bell_MF(MF_object):
    ...
    def __init__(self, a, b, c):
        ...
        if a <= 0:
            print(
                "WARNING: sigma should be strictly positive, Simpful received sigma=%f"
                % a
            )
...
class Gaussian_MF(MF_object):
    ...
    def __init__(self, mu, sigma):
        ...
        if sigma <= 0:
            print(
                f"WARNING: sigma should be strictly positive, Simpful received sigma={sigma}"
            )
...
class InvGaussian_MF(MF_object):
    ...
    def _execute(self, x):
        return 1.0 - _gaussian(x, self._mu, self._sigma)
...
class DoubleGaussian_MF(MF_object):
    ...
    def __init__(self, mu1, sigma1, mu2, sigma2):
        ...
        if sigma1 <= 0:
            print(
                "WARNING: sigma should be strictly positive, Simpful received sigma1=%f"
                % sigma1
            )
        self._mu2 = mu2
        self._sigma2 = sigma2
        if sigma2 <= 0:
            print(
                "WARNING: sigma should be strictly positive, Simpful received sigma2=%f"
                % sigma2
            )
...
class FuzzySet(object):
    ...
    def __init__(
        ...
        for p in points:
            if len(p) > 2:
                raise Exception(
                    'ERROR: one fuzzy set named "'
                    + self._term
                    + '" has more than two coordinates.'
                )
        ...
        if boundary_values == None:
            ...
            if len(boundary_values) == 2 and all(
                isinstance(x, (int, float)) for x in boundary_values
            ):
    ...
    def get_value_slow(self, v):
        f = interp1d(
            ...
            self._points.T[1],
            bounds_error=False,
            fill_value=(self.boundary_values[0], self.boundary_values[1]),
        )
        result = f(v)
        return result
    ...
    def get_value_fast(self, v):
        ...
        if v < x[0]:
            return self.boundary_values[
                0
            ]  # fallback for values outside the Universe of the discourse
        for i in range(N - 1):
    ...
    def set_params(self):
        print(
            "Attention: this is a virtual method for setting parameters of pre-baked fuzzy sets."
        )
    ...
    def set_points(self, points):
        ...
        if self._type == "function":
            print(
                'WARNING: the fuzzy set named "'
                + self._term
                + '" was converted from function-based to point-based.'
            )
        for p in points:
            if len(p) > 2:
                raise Exception(
...
class TriangleFuzzySet(FuzzySet):
    ...
    def set_params(self, a=None, b=None, c=None):
        ...
        """
        if a is not None:
            self._funpointer._a = a
        if b is not None:
            self._funpointer._b = b
        if c is not None:
            self._funpointer._c = c

    def get_params(self):
        return [
            "TriangularMembFunc",
            (self._funpointer._a, self._funpointer._b, self._funpointer._c),
        ]
...
class GaussianFuzzySet(FuzzySet):
    ...
    def set_params(self, mu=None, sigma=None):
        ...
        """
        if mu is not None:
            self._funpointer._mu = mu
        if sigma is not None:
            self._funpointer._sigma = sigma
...
class DoubleGaussianFuzzySet(FuzzySet):
    ...
    def set_params(self, mu1=None, sigma1=None, mu2=None, sigma2=None):
        ...
        """
        if mu1 is not None:
            self._funpointer._mu1 = mu1
        if sigma1 is not None:
            self._funpointer._sigma1 = sigma1
        if mu2 is not None:
            self._funpointer._mu2 = mu2
        if sigma2 is not None:
            self._funpointer._sigma2 = sigma2

    def get_params(self):
        return [
            "DoubleGaussMembFunc",
            (
                self._funpointer._mu1,
                self._funpointer._sigma1,
                self._funpointer._mu2,
                self._funpointer._sigma2,
            ),
        ]
...
class CrispSet(FuzzySet):
    ...
    def set_params(self, a=None, b=None):
        ...
        """
        if a is not None:
            self._funpointer._left = a
        if b is not None:
            self._funpointer._right = b
