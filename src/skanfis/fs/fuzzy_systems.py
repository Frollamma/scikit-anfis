...
class LinguisticVariable(object):
    ...
    def draw(self, ax, TGT=None, highlight=None, xscale="linear"):
        ...
        if xscale == "linear":
            ...
            if mi < 0 and ma > 0:
                x = geomspace(mi, -finfo(float64).eps, 5000) + geomspace(
                    finfo(float64).eps, ma, 5000
                )
        ...
        if TGT is not None:
            ax.axvline(
                x=TGT, ymin=0.0, ymax=1.0, color="red", linestyle="--", linewidth=2.0
            )
    ...
    def __repr__(self):
        ...
        return (
            "<Linguistic variable '"
            + text
            + "', contains fuzzy sets %s, universe of discourse: %s>"
...
class AutoTriangle(LinguisticVariable):
    ...
    def __init__(
        self, n_sets=3, terms=None, universe_of_discourse=[0, 1], verbose=False
    ):

        if n_sets < 2:
            raise Exception(
                "Cannot create linguistic variable with less than 2 fuzzy sets."
            )
        ...
        FS_list.append(
            FuzzySet(function=Triangular_MF(low, low, control_points[1]), term=terms[0])
        )
            ...
            FS_list.append(
                FuzzySet(
                    function=Triangular_MF(
                        control_points[n - 1], control_points[n], control_points[n + 1]
                    ),
                    term=terms[n],
                )
            )
        ...
        FS_list.append(
            FuzzySet(
                function=Triangular_MF(control_points[-2], high, high), term=terms[-1]
            )
        )
...
class FS(BaseFuzzySystem, BaseEstimator, TransformerMixin):
    ...
    def __init__(
        self,
        _operators="AND_PRODUCT",
        _antec={},
        _coeff=[],
        _detected_type=None,
        _lvs=OrderedDict(),
        _variables=OrderedDict(),
        _crispvalues=OrderedDict(),
        _outputfunctions=OrderedDict(),
        _outputfuzzysets=OrderedDict(),
        _outputvariables=[],
        _constants=[],
        _rules=None,
        _sanitize_input=False,
        verbose=True,
    ):
    ...
    def outvaribles(self):
        array_rules = array(self._rules, dtype="object")
        self._outputvariables = list(
            set([n[0] for _, n in enumerate(array_rules.T[1])])
        )
    ...
    def get_fuzzy_sets(self, variable_name):
        ...
        try:
            ...
            raise Exception(
                "ERROR: linguistic variable %s does not exist" % variable_name
            )
    ...
    def get_fuzzy_set(self, variable_name, fs_name):
        ...
        raise Exception(
            "ERROR: fuzzy set %s of linguistic variable %s does not exist"
            % (fs_name, variable_name)
        )
    ...
    def set_constant(self, name, value, verbose=False):
        ...
        try:
            ...
            if verbose:
                print(" * Variable %s set to a constant value %f" % (name, value))
        except ValueError:
            raise Exception(
                "ERROR: specified value for "
                + name
    ...
    def add_rules(self, rules, verbose=False):
        ...
        for rule in rules:
            ...
            parsed_antecedent = recursive_parse(
    ...
    def _set_model_type(self, model_type):
        if self._detected_type == "inconsistent":
            return
        if self._detected_type is None:
            ...
            print(
                "WARNING: model type is unclear (simpful detected %s, but a %s output was specified)"
                % (self._detected_type, model_type)
            )
    ...
    def get_firing_strengths(self, input_values=None):
        ...
        if input_values is None:
            results = [
                float(antecedent[0].evaluate(self)) for antecedent in self._rules
            ]
            return results
        else:
            firings = []
                ...
                if outname == output:
                    if outterm not in list_crisp_values:
                        ...
                        if outterm not in list_output_funs:
                            raise Exception(
                                "ERROR: one rule calculates an output named '"
                                + outterm
                    ...
                    if crisp:
                        ...
                        for k, v in self._variables.items():
                            ...
                            string_to_evaluate = re.sub(
                                r"(?P<front>\W|^)" + k + r"(?P<end>\W|$)",
                                r"\g<front>" + str(v) + r"\g<end>",
                                string_to_evaluate,
                            )
                    ...
                    try:
                        ...
                        raise Exception(
                            "ERROR: one rule could not be evaluated\n"
                            + " --- PROBLEMATIC RULE:\n"
                            + "IF "
                            + str(ant)
                            + " THEN "
            ...
            try:
                if den == 0.0:
                    ...
                    if not ignore_warnings:
                        print(
                            "WARNING: the sum of rules' firing for variable '%s' is equal to 0. The result of the Sugeno inference was set to 0."
                            % output
                        )
                else:
                ...
                if ignore_errors:
                    print(
                        "WARNING: cannot perform Sugeno inference for variable '%s'. The variable appears only as antecedent in the rules or an arithmetic error occurred."
                        % output
                    )
                else:
                    raise Exception(
                        "ERROR: cannot perform Sugeno inference for variable '%s'. The variable appears only as antecedent in the rules or an arithmetic error occurred."
                        % output
                    )
    ...
    def mediate_Mamdani(
        ...
        for output in outputs:
            if verbose:
                print(" * Processing output for variable '%s'" % output)
                print(
                    "   whose universe of discourse is:",
                    self._lvs[output].get_universe_of_discourse(),
                )
                print(
                    "   contains the following fuzzy sets:", self._lvs[output]._FSlist
                )
            ...
            if verbose:
                print(" * Indices:", convenience_dict)
            ...
            try:
                if sumv == 0.0:
                    ...
                    if not ignore_warnings:
                        print(
                            "WARNING: the sum of rules' firing for variable '%s' is equal to 0. The result of the Mamdani inference was set to 0."
                            % output
                        )
                else:
                    CoG = sumwv / sumv
                ...
                if ignore_errors:
                    print(
                        "WARNING: cannot perform Mamdani inference for variable '%s'. The variable appears only as antecedent in the rules or an arithmetic error occurred."
                        % output
                    )
                else:
                    raise Exception(
                        "ERROR: cannot perform Mamdani inference for variable '%s'. The variable appears only as antecedent in the rules or an arithmetic error occurred."
                        % output
                    )

            if verbose:
                print(
                    " * Weighted values: %.2f\tValues: %.2f\tCoG: %.2f"
                    % (sumwv, sumv, CoG)
                )
    ...
    def Sugeno_inference(
        self, terms=None, ignore_errors=False, ignore_warnings=False, verbose=False
    ):
        """
        ...
        if terms == None:
            ...
            for t in terms:
                if t not in set([rule[1][0] for rule in self._rules]):
                    raise Exception(
                        "ERROR: Variable " + t + " does not appear in any consequent."
                    )
    ...
    def inference(
        ...
        if self._detected_type == "Sugeno":
            ...
            return self.probabilistic_inference(
                terms=terms,
                ignore_errors=ignore_errors,
                ignore_warnings=ignore_warnings,
                verbose=verbose,
    ...
    def plot_variable(
        ...
        if ax != None:
            ax = self._lvs[var_name].draw(
                ax=ax, TGT=TGT, highlight=highlight, xscale=xscale
            )
            return ax
        self._lvs[var_name].plot(
            outputfile=outputfile, TGT=TGT, highlight=highlight, xscale=xscale
        )
    ...
    def predict(self, X, round=True):
        """
        If round is true it performs abs and then rounding, otherwise it returns raw results
        """
        # print('fuzzy predict')
        # print(X[0])
